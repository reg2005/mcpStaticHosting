...
export function readConfig(env: NodeJS.ProcessEnv) {
  ...
  if (!/^[a-z0-9]+$/.test(provider) || provider === "exec" || provider === "manual") throw new Error("Укажите DNS_PROVIDER с автоматической поддержкой DNS API");
  ...
  return { main, email, provider, server, resolvers, cidrs: parseCidrs(env.MANAGEMENT_ALLOWED_CIDRS ?? ""), credentialsFile: env.DNS_CREDENTIALS_FILE ?? "/run/secrets/dns_credentials", ipv4: env.PUBLIC_IPV4 || null, ipv6: env.PUBLIC_IPV6 || null };
}
