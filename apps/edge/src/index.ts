...
let lastDiscovery = 0;
let wildcardRetry = { attempts: 0, next: 0, server: config.server };
try { wildcardRetry = JSON.parse(await readFile("/edge/wildcard-retry.json", "utf8")); } catch { /* First boot. */ }
if (wildcardRetry.server !== config.server) wildcardRetry = { attempts: 0, next: 0, server: config.server };
...
async function reconcileWildcard() {
  let cert = await certificateState(config.main, config.server, true);
  if ((!cert.valid || cert.renew) && Date.now() >= wildcardRetry.next) {
    log("WILDCARD_ACME_START", config.main);
    try {
      await issueCertificate(config, config.main, true, cert.valid);
      cert = await certificateState(config.main, config.server, true);
      wildcardRetry = { attempts: 0, next: Date.now() + 3600000, server: config.server };
      log("WILDCARD_ACME_READY", config.main);
    } catch {
      wildcardRetry = { attempts: wildcardRetry.attempts + 1, next: Date.now() + retryDelay(wildcardRetry.attempts), server: config.server };
      log("WILDCARD_ACME_RETRY_CHECK_DNS_CREDENTIALS", config.main);
    }
...
async function cycle() {
  ...
  await reload();
  await db.insert(instanceState).values({ id: "edge", value: { ...addresses, wildcardTls: wildcard ? "active" : "pending", error }, updatedAt: new Date() })
    .onConflictDoUpdate({ target: instanceState.id, set: { value: { ...addresses, wildcardTls: wildcard ? "active" : "pending", error }, updatedAt: new Date() } });
...
wildcard = (await certificateState(config.main, config.server, true)).valid;
...
const health = createServer((req, res) => {
  ...
  res.end(JSON.stringify({ ok: alive, wildcardTls: wildcard ? "active" : "pending" }));
});
